...
class InsertDEVCApp(QMainWindow):
    ...
    def on_apply(self):
        ...
        ini_quantity_file = f"IniQuantity_{self.unique_id}.ini" if self.unique_id else "IniQuantity.ini"
        self.write_to_ini(ini_quantity_file, "IniQuantity", "Quantity", quantity)

        # Сохранение высоты в INI файл
        ini_hz_file = f"IniHZ_{self.unique_id}.ini" if self.unique_id else "IniHZ.ini"
        self.write_to_ini(ini_hz_file, "IniHZ", "HZ", self.hz_input.text())
    ...
    def calculate_fpom(self, file_path):
        """Вычисление fpom на основе параметров MESH из FDS файла."""
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                fds_contents = f.read()
        except Exception as e:
            QMessageBox.critical(self, "Ошибка", f"Не удалось прочитать файл для вычисления fpom: {str(e)}")
            return None

        # Поиск всех строк &MESH и извлечение IJK и XB
        mesh_pattern = r'&MESH\s+([^/]+)/'
        mesh_matches = re.findall(mesh_pattern, fds_contents)

        if not mesh_matches:
            return None

        # Сбор всех значений IJK и XB
        ijk_values = []
        xb_values = []

        for mesh_data in mesh_matches:
            # Извлечение IJK (порядок не важен)
            ijk_match = re.search(r'IJK=(\d+),(\d+),(\d+)', mesh_data)
            if ijk_match:
                ijk_values.append((int(ijk_match.group(1)), int(ijk_match.group(2)), int(ijk_match.group(3))))

            # Извлечение XB (порядок не важен)
            xb_match = re.search(r'XB=([-\d\.]+),([-\d\.]+),([-\d\.]+),([-\d\.]+),([-\d\.]+),([-\d\.]+)', mesh_data)
            if xb_match:
                xb_values.append((
                    float(xb_match.group(1)), float(xb_match.group(2)),  # X1, X2
                    float(xb_match.group(3)), float(xb_match.group(4)),  # Y1, Y2
                    float(xb_match.group(5)), float(xb_match.group(6))   # Z1, Z2
                ))

        if not ijk_values or not xb_values or len(ijk_values) != len(xb_values):
            return None

        # Вычисление размера ячейки (Cs) по вертикали (K и Z)
        cell_sizes = []
        for (i, j, k), (x1, x2, y1, y2, z1, z2) in zip(ijk_values, xb_values):
            delta_z = abs(z2 - z1) / k
            cell_sizes.append(delta_z)

        # Используем минимальный размер ячейки (наиболее консервативный подход)
        if not cell_sizes:
            return None

        Cs = min(cell_sizes)

        # Вычисление fpom по формуле
        # L = R * sqrt(2), где R - радиус зоны обнаружения (принимаем равным Cs для упрощения)
        # fpom = ceil(pi * (L**2) / 4)
        # Cc = ceil(fpom/Cs)

        # Принимаем R = Cs (размер ячейки)
        R = Cs
        L = R * math.sqrt(2)
        fpom_raw = math.pi * (L ** 2) / 4
        fpom = math.ceil(fpom_raw)
