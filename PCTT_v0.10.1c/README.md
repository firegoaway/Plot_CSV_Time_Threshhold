...
Фактически:
Пусть ${ \mathsf{L} }$ – множество величин с размерностью длины. Тогда для любой величины $Q \in { \mathsf{L}^{-1} }$ справедливо:
$$
\dim(Q) = \mathsf{L}^{-1}
\text{Существует отображение} \varphi: \mathbb{R}{>0} \times \mathbb{R}{>0} \to \mathbb{R} \text{, такое что} \varphi(A, A_0) = \ln(A/A_0)\text{ и } \varphi \in {\text{Нп}}
\text{Следовательно, единица измерения } [Q] = 1 \text{ м}^{-1}\text{ и }[Q] = 1 \text{ Нп/м}\text{ являются метрически тождественными } (1 \text{ м}^{-1} \equiv 1 \text{ Нп/м}) \text{ в силу безразмерности } \text{Нп} = [\ln(X/X_0)] = 1
\text{Таким образом:}
\dim(Нп) = 1
$$
...
$${f'\left(\frac{1}{x}\right)} = \int\frac{1}{x}{dx} = \ln(x) + {C} \quad \text{при} \hspace{2mm} x\in\left(0;+\infty\right)$$
...
> **PCTT** работает только в связке с утилитой [**Zmejka**](https://github.com/firegoaway/Zmejka)
