...
Преобразование 0.2 дБ/м –> 1/м для подстановки во входной файл `.fds` осуществляется следующим образом:
...
$1 \text{ м}^{-1}$ – единица СИ, порождающаяся естественным образом при интегрировании ОДУ 1-порядка в FDS в качестве необратимой постоянной $\text{C}_{1}$ слева.

Согласно междунарожному стандарту **ISO 80000-3**, $\text{Нп}$ является единицей логарифмического отношения двух величин одного рода (одной модальности).

Формально: ${L} = \ln(\frac{X_{1}}{X_{2}})  \text{Нп}$
...
В теории размерностей Г.Г. Филлипов в своей книге "Теория размерностей и LTM-физика" называет Нп – когерентной производной единицей размерности ${1}$ (безразмерная).

Фактически: $\dim(Нп) = 1$

Таким образом, $1 \text{ Нп/м}$ – единица логарифмического отношения с производной вида $f'(\frac{1}{x}) = \int\frac{1}{x}dx = \ln(X)+C$.

Это фундаментальное свойство делает неперы чрезвычайно удобными для практической работы с FDS.

Поскольку единица измерения в  FDS (м⁻¹) численно равна неперу на метр, нам не нужно переводить из Нп/м в 1/м или обратно. Если в нормативном документе порог указан в Нп/м – мы просто копируем это числовое значение в параметр `SETPOINT` без каких-либо изменений.
