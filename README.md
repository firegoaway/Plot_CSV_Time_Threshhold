...
Формально: ${L} = \ln\left(\frac{X_{1}}{X_{2}}\right) \qquad \text{Нп}$
...
Таким образом, $1 \text{ Нп/м}$ – единица логарифмического отношения с производной вида

$${f'\left(\frac{1}{x}\right)} = \int\frac{1}{x}{dx} = \ln(x) + {C}$$

Это фундаментальное свойство делает неперы чрезвычайно удобными для практической работы с FDS. так как Нм/м образует с размерностью 1/м метрическое тождество.
